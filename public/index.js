...
//materials
var mat_orange = new THREE.MeshLambertMaterial({ color: 0xf3f2f7 });
var mat_grey = new THREE.MeshLambertMaterial({ color: 0xa3785f });
var mat_yellow = new THREE.MeshLambertMaterial({ color: 0x00ff00 });
...
var mat_blanco = new THREE.MeshLambertMaterial({ color: 0xffffff });
// Material para el cuerpo de la gallina
var bodyMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb });

// Material para la cabeza de la gallina
var headMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb });

// Material para los ojos de la gallina
var eyeMaterial = new THREE.MeshLambertMaterial({ color: 0xffffff });

// Material para las pupilas de los ojos de la gallina
var eyeballMaterial = new THREE.MeshLambertMaterial({ color: 0x000000 });

// Material para el pico de la gallina
var beakMaterial = new THREE.MeshLambertMaterial({ color: 0xffc000 });

var eyeballGeometry = new THREE.MeshBasicMaterial({ color: 0xffffff });
//-------------------------------------ground-------------------------------------
...
for (var i = 0; i < 5; i++) {
  var h = 0.1;
  var geometry = new THREE.CylinderGeometry(15 - i - 0.01, 15 - i, h, 9);
  ...
  layers.push(new THREE.Mesh(geometry, material));
...
mainStem.scale.set(0.002, 0.007, 0.6);
...
stone[1].rotation.set(0, 0, pi / 2);
stone[1].scale.set(1, 1, 1);
stone[1].position.set(0, 0.7, 7.3);

//-------------------------------------sheep-------------------------------------
//sheep body
var sheep = new THREE.Group();
var geo_sheepHead = new THREE.SphereGeometry(0.5, 8, 6);
...
var sheepBody = new THREE.Mesh(geo_sheepBody, mat_blanco);
...
var tail = new THREE.Mesh(geo_tail, mat_blanco);
...
for (var i = 0; i < 5; i++) {
  hair[i] = new THREE.Mesh(geo_hair, mat_blanco);
...
var eyes = [];
for (var i = 0; i < 2; i++) {
  eyes[i] = new THREE.Mesh(geo_eye, mat_blanco);
  sheep.add(eyes[i]);
...
var bodyMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb }); // Color rosa claro para el cuerpo
var body = new THREE.Mesh(bodyGeometry, bodyMaterial);
chicken.add(body);

// Cabeza de la gallina
var headGeometry = new THREE.BoxGeometry(9, 9, 9);
var headMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb }); // Color rosa claro para la cabeza
var head = new THREE.Mesh(headGeometry, headMaterial);
head.position.y = 11;
chicken.add(head);

// Ojos
var eyeGeometry = new THREE.CylinderGeometry(0.5, 0.5, 0.5, 8);
var eyeMaterial = new THREE.MeshLambertMaterial({ color: 0xffffff }); // Color blanco para los ojos
var eyes = [];
for (var i = 0; i < 2; i++) {
  eyes[i] = new THREE.Mesh(eyeGeometry, eyeMaterial);
  head.add(eyes[i]);
  eyes[i].castShadow = true;
  eyes[i].position.set(2.5 * (i === 0 ? 1 : -1), 1, 4.5);
  eyes[i].rotation.x = Math.PI / 2 - Math.PI / 15;
}
/* eyes[0].position.x = 1.25;
eyes[1].position.x = -1.25;
eyes[0].rotation.z = -Math.PI / 15;
eyes[1].rotation.z = -eyes[0].rotation.z; */

// Pupilas
var eyeballGeometry = new THREE.SphereGeometry(0.055, 8, 8);
var eyeballMaterial = new THREE.MeshLambertMaterial({ color: 0x000000 }); // Color negro para las pupilas
var eyeballs = [];
for (var i = 0; i < 2; i++) {
  eyeballs[i] = new THREE.Mesh(eyeballGeometry, eyeballMaterial);
  eyes[i].add(eyeballs[i]);
  eyeballs[i].castShadow = true;
  eyeballs[i].position.z = 0.01;
}

// Pico
var beakGeometry = new THREE.ConeGeometry(2, 6, 8);
var beakMaterial = new THREE.MeshLambertMaterial({ color: 0xffc000 }); // Color amarillo para el pico
...
// Patas
var legGeometry = new THREE.BoxGeometry(2, 10, 2);
var legMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb }); // Color rosa claro para las patas
var legs = [];
for (var i = 0; i < 2; i++) {
  legs[i] = new THREE.Mesh(legGeometry, legMaterial);
  legs[i].position.y = -7.5;
  legs[i].position.x = i === 0 ? -4 : 4;
  chicken.add(legs[i]);
}

// Plumas
var featherGeometry = new THREE.BoxGeometry(1, 3, 0.2); // Geometría de la pluma
var featherMaterial = new THREE.MeshLambertMaterial({ color: 0xaaaaaa }); // Material de la pluma

// Crear y posicionar las plumas
var feathers = [];
for (var i = 0; i < 10; i++) {
  var feather = new THREE.Mesh(featherGeometry, featherMaterial);
  feather.position.y = 5;
  feather.position.x = Math.random() * 2 - 1; // Posición horizontal aleatoria
  feather.position.z = Math.random() * 1 + 2.5; // Posición en profundidad aleatoria
  feather.rotation.z = Math.random() * Math.PI * 2; // Rotación aleatoria
  head.add(feather);
  feathers.push(feather);
}

// Alas
var wingGeometry = new THREE.BoxGeometry(10, 1, 4);
var wingMaterial = new THREE.MeshLambertMaterial({ color: 0xffc0cb }); // Color rosa claro para las alas
var wings = [];
for (var i = 0; i < 2; i++) {
  wings[i] = new THREE.Mesh(wingGeometry, wingMaterial);
  wings[i].position.y = 5;
  wings[i].position.x = i === 0 ? -6 : 6;
  wings[i].rotation.z = Math.PI / 4;
  chicken.add(wings[i]);
}

// Posicionamiento y escala de la gallina
chicken.position.set(7, 1.5, 10); // Cambia las coordenadas (0, 0, 0) por la posición deseada
chicken.scale.set(0.1, 0.1, 0.1); // Cambia los valores (2.5, 2.5, 2.5) por la escala deseada

// Añadir la gallina a la escena
scene.add(chicken);
...
wood[3].rotation.x = pi / 400;
wood[2].rotation.x = -pi / 400;
wood[2].rotation.y = pi / 500;

fence.position.set(1, 0, 13);
fence.rotation.y = pi / 3;
scene.add(fence);
...
fence.position.set(-2, -0.01, 11.51);
fence.rotation.y = pi / 3;
scene.add(fence);
//fence4
var fence = new THREE.Group();
